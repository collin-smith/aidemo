...
const DocumentAnalysis: React.FunctionComponent<IHomeProps> = () => {
        ...
        if (configured==false && objects !=null)
          {
            ...
            for (let i = 0; i < objectArray.length; i++) {
              ...
              if (im.key.endsWith(".pdf"))
              {

                let newLabel = im.key.substring(0,im.key.length)
                obj.label = newLabel
                list.push(obj)
              }
            }
  ...
  const handleClick = async () => {
    ...
    {
      const restURL = configurationProperties.baseUrl+'/prod/documentanalysisllm';
  ...
  const handleSelectChange = (index: number) => {
   ...
   // console.log("after selected option="+JSON.stringify(selectedDropdownOption))
    ...
    return (
        ...
        <p className="font-bold">Submitting the actual PDF Document to LLM in Base64 format</p>

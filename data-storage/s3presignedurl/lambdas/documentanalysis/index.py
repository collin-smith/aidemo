...
def handler(event, context):
    ...
    try:
        try:
            ...
            #Retrieve the contents of the Textract Generated Text
            #Let's look up the S3 object and get the contents
            textract = boto3.client('textract')
            s3 = boto3.client('s3')
            response = s3.get_object(Bucket=s3bucketname, Key=output_key)
            # The object content is in the 'Body' stream
            text = response['Body'].read().decode('utf-8') # Decode for text files            

            #Execute Prompt against the text
            prompt = f""+prompt+":\n\n"+text+"\n\n"
            results = ""
    ...
    "messages": [
      {
        ...
        "content": [

          {
            ...
            logger.info("calling bedrock now")
            ...
            analysis = response.get("body").read().decode('utf-8')
            resultobject = json.loads(analysis)
        ...
        items = []
        row_data = {}
        row_data["call"] = "documentanalysis"
        row_data["key"] = key

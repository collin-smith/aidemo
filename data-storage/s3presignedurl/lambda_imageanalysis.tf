data "archive_file" "lambda_imageanalysis_zip" {
  type = "zip"
source_dir  = "${path.module}/lambdas/imageanalysis/"
output_path = "${path.module}/lambdas/imageanalysis.zip"
}

resource "aws_lambda_function" "lambda_imageanalysis" {
  function_name = "imageanalysis"
  role          = aws_iam_role.iamrole_fullaccess.arn
  handler       = "index.handler"
  runtime       = var.python_version
  filename         = data.archive_file.lambda_imageanalysis_zip.output_path
  source_code_hash = data.archive_file.lambda_imageanalysis_zip.output_base64sha256
  timeout = 28
  memory_size = 1024

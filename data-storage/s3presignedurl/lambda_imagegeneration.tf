data "archive_file" "lambda_imagegeneration_zip" {
  type = "zip"
source_dir  = "${path.module}/lambdas/imagegeneration/"
output_path = "${path.module}/lambdas/imagegeneration.zip"
}

resource "aws_lambda_function" "lambda_imagegeneration" {
  function_name = "imagegeneration"
  role          = aws_iam_role.iamrole_fullaccess.arn
  handler       = "index.handler"
  runtime       = var.python_version
  filename         = data.archive_file.lambda_imagegeneration_zip.output_path
  source_code_hash = data.archive_file.lambda_imagegeneration_zip.output_base64sha256
  timeout = 28
  memory_size = 1024
